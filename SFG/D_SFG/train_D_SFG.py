...
from reader.ANHIR import LoadANHIR, ANHIRPredict, LoadANHIR_without_train
import cv2
...
if dataset_cfg.dataset.value == 'ANHIR':
	...
	print(subset)
	sys.stdout.flush()
	dataset, groups, groups_train, groups_val = LoadANHIR_without_train(args.prep, subset)
...
# sys.stdout.flush()
if dataset_cfg.dataset.value == "ANHIR":
    raw_shape = dataset[eval_pairs[0][0]].shape[1: 3]
else:
...
prep_name = args.prep
data_path = r"/data/gl/re_do_from_ori/data/data_for_deformable_network"
...
def iterate_data(iq, gen):
	...
	while True:
		...
		if dataset_cfg.dataset.value == "ANHIR":
			#print('next(gen)', i)
			#print('iq,put', train_pairs[i])
			##
			out_to_iqput = []

			fimg2 = train_pairs[i][0]
			fimg1 = train_pairs[i][1]
			flmk2 = train_pairs[i][2]
			flmk1 = train_pairs[i][3]
			fsift2 = train_pairs[i][4]
			fsift1 = train_pairs[i][5]


			im_temp21 = io.imread(os.path.join(prep_path1, fimg2), as_gray=True)
			im_temp22 = np.zeros((3, np.shape(im_temp21)[0], np.shape(im_temp21)[1]))
			im_temp22[0] = im_temp21
			im_temp22[1] = im_temp21
			im_temp22[2] = im_temp21
			# o_0 = im_temp2
			out_to_iqput.append(im_temp22)

			im_temp11 = io.imread(os.path.join(prep_path1, fimg1), as_gray=True)
			im_temp12 = np.zeros((3, np.shape(im_temp11)[0], np.shape(im_temp11)[1]))

			im_temp12[0] = im_temp11
			im_temp12[1] = im_temp11
			im_temp12[2] = im_temp11
			# o_1 = im_temp12
			out_to_iqput.append(im_temp12)

			try:
				lmk = pd.read_csv(os.path.join(prep_path, flmk2))
				lmk = np.array(lmk)
				lmk2 = lmk[:, [2, 1]]

				while np.shape(lmk2)[0] > 200:
					print('pair num > 200')
					lmk2 = lmk2[::2, :]
				lmk2 = np.pad(lmk2, ((0, 200 - len(lmk2)), (0, 0)), "constant")
			except:
				lmk2 = np.zeros((200, 2), dtype=np.int64)
			# o_2 = copy.deepcopy(lmk2)
			out_to_iqput.append(lmk2)

			try:
				lmk = pd.read_csv(os.path.join(prep_path, flmk1))
				lmk = np.array(lmk)
				lmk1 = lmk[:, [2, 1]]

				while np.shape(lmk1)[0] > 200:
					print('pair num > 200')
					lmk1 = lmk1[::2, :]
				lmk1 = np.pad(lmk1, ((0, 200 - len(lmk1)), (0, 0)), "constant")
			except:
				lmk1 = np.zeros((200, 2), dtype=np.int64)
			# o_3 = lmk1
			out_to_iqput.append(lmk1)

			siftdata = scio.loadmat(os.path.join(siftdir, fsift2))
			sift2 = siftdata[fsift2[-9:-4] + 'new']
			out_to_iqput.append(sift2)

			siftdata = scio.loadmat(os.path.join(siftdir, fsift1))
			sift1 = siftdata[fsift1[-9:-4]+'new']
			out_to_iqput.append(sift1)
			...
			##
			# out_to_iqput.append(o_0)
			# # print(train_pairs[i][0])
			# # print(np.shape(o_0))
			# out_to_iqput.append(o_1)
			# # print(train_pairs[i][1])
			# # print(np.shape(o_1))
			# out_to_iqput.append(o_2)
			# # print(train_pairs[i][2])
			# # print(np.shape(o_2))
			# out_to_iqput.append(o_3)
			# # print(train_pairs[i][3])
			# # print(np.shape(o_3))
			# out_to_iqput.append(o_4)
			# # print(train_pairs[i][4])
			# # print(np.shape(o_4))
			# out_to_iqput.append(o_5)
			# # print(train_pairs[i][5])
			# # print(np.shape(o_5))
