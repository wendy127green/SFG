...
def main():
    ...
    dropthreshold_multiscale = [0.95,0.8,0.8]
    zone_ratio = 0.04*0.04
    max_num = 1000
    resultdir = r'/data2/gl/wxy_data/kp_result512_multiscale/threshold' + str(dropthreshold_multiscale) + 'zone' + str(
        np.sqrt(zone_ratio)) + 'without_del_mice_kidney_csv'
    imgfile = r'/data2/gl/wxy_data/512split_8000after_affine/'
    newdir = os.path.join(resultdir, 'oriresult_512_multiscale')  # r'.\orb_kp_siftflow_desc_result\oriresult_512'
    newdirv = os.path.join(resultdir, 'visualization')  # r'.\orb_kp_siftflow_desc_result\visualization'
    matrix_seq = pd.read_csv(r'/data2/gl/wxy_data/matrix_sequence_manual_validation.csv')
    matrix_seq = np.array(matrix_seq)
    siftflowdir1 = r'/data2/gl/wxy_data/outsift_512'
    siftflowdir2 = r'/data2/gl/wxy_data/outsift_512_256'
    siftflowdir3 = r'/data2/gl/wxy_data/outsift_512_128'
    ...
    for i in range(0, 500):
        ...
        if matrix_seq[i][5] == 'training':
            for j in range(0, 64):
                for k in range(0, 4):
                    siftflow1name = str(i) + '_1_' + str(j) + '_' + str(k) + 'sift1.mat'
                    siftflow2name = str(i) + '_2_' + str(j) + '_' + str(k) + 'sift2.mat'
                    sdir1 = os.path.join(siftflowdir1, siftflow1name)
                    sdir2 = os.path.join(siftflowdir1, siftflow2name)
                    if os.path.exists(sdir1) and os.path.exists(sdir2) and count_num<=15:
                        count_num=count_num+1
                        print('pairs:', i)
                        img1name = str(i) + '_1_' + str(j) + '_' + str(k) + '.jpg'
                        lmk1name = str(i) + '_1_' + str(j) + '_' + str(k) + '.csv'
                        img2name = str(i) + '_2_' + str(j) + '_' + str(k) + '.jpg'
                        lmk2name = str(i) + '_2_' + str(j) + '_' + str(k) + '.csv'
                        matchname1 = 'match_' + str(i) + '_1_' + str(j) + '_' + str(k) + '.png'
                        matchname2 = 'match_' + str(i) + '_2_' + str(j) + '_' + str(k) + '.png'
                        ...
                        # while True:
                        matches, scorevalues = multiscale_match_twosided(siftflow1name,siftflow2name, siftflowdir1, siftflowdir2, siftflowdir3, filtered_coords1, filtered_coords2, imsize, dropthreshold_multiscale, zone_ratio)

                        no0 = np.flatnonzero(matches)
                        #print('pair {0} match num: {1}'.format(i, len(no0)))
                        matchlocs1, matchlocs2 = plot_matches(gray1, gray2, filtered_coords1, filtered_coords2, matches, scorevalues, name1, name2, max_num, maskfile, i)
...
def plot_matches(im1, im2, locs1, locs2, matchscores, scorevalues, name1, name2, max_num, maskfile, imgname):
...
def multiscale_match(sift12flip, siftflow1name,siftflow2name, siftflowdir1, siftflowdir2, siftflowdir3, kp1, kp2, imsize,dotprod_threshold, zone_ratio):
    ...
    for i in range(desc1_size[1]):
        ...
        if dist_ratio == 0 and len(indx) > 0: # only using threshold, not using ratio
            ...
            if dotprods1[indx[0]] > dotprod_threshold[0]:
                for subind in range(1, 5):
                    if dotprods1[indx[subind]] > 0.90*dotprods1[indx[0]]:
                        if (kp2[indx[subind]][0]-kp2[indx[0]][0])*(kp2[indx[subind]][0]-kp2[indx[0]][0])+(kp2[indx[subind]][1]-kp2[indx[0]][1])*(kp2[indx[subind]][1]-kp2[indx[0]][1])> 0.005*0.005 * (imsizex * imsizex + imsizey * imsizey):
                ...
                # print(dotprods1[indx[0]])
                if dotprods1[indx[0]] > dotprod_threshold[0] and dotprods[indx[0]][1] > dotprod_threshold[1] and dotprods[indx[0]][2] > dotprod_threshold[2]:
                    contnum = contnum + 1
                    matchscores[i] = int(indx[0])
...
def multiscale_match_twosided(siftflow1name,siftflow2name, siftflowdir1, siftflowdir2, siftflowdir3, filtered_coords1, filtered_coords2, imsize,dropthreshold,zone_ratio):
    """ Two-sided symmetric version of match(). """
    sift12flip = 0
    matches_12, score_12 = multiscale_match(sift12flip, siftflow1name, siftflow2name, siftflowdir1, siftflowdir2, siftflowdir3, filtered_coords1, filtered_coords2, imsize, dropthreshold, zone_ratio)
    sift12flip = 1
    matches_21, score_21 = multiscale_match(sift12flip,siftflow1name,siftflow2name, siftflowdir1, siftflowdir2, siftflowdir3, filtered_coords2, filtered_coords1, imsize, dropthreshold, zone_ratio)
    ndx_12 = matches_12.nonzero()[0]
    # delet the asymmetry matches
