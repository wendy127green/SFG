###################Training
To train S_SFG (Sparse SFG), run "python train_S_SFG.py MaskFlownet_S.yaml --dataset_cfg ANHIR.yaml -g 0 -c 2afApr28 --clear_steps --weight 200 --batch 1 --relative UM --prep 512". 

###################Validation
To validate S_SFG (Sparse SFG), run "python train_S_SFG.py MaskFlownet_S.yaml --dataset_cfg ANHIR.yaml -g 0 -c 2afApr28 --clear_steps --weight 200 --batch 1 --relative UM --prep 512 --valid". 
...
(2)The /datasets/512_kp_after_affine includes the auto-obtained key points for training images used in training process. Please go to /SFG/Automatic_key_point_extraction/ to see the code to obtain the key points.
